#' @export
plotBoatLengths <- function(s,fp){
  fp$LOA = round(fp$LOA)
  f1 = as.data.frame(table(fp$LOA)/nrow(fp)*100)
...
print(  ggplot(xr,aes(fill=time,y=Fishery,x=Length))+geom_bar(position = 'dodge',stat='identity')+labs(x = 'Boat Length (ft)', y='Percent'))
  return(xr)
}
